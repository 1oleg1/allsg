        ...
        if($this->dynamicTranslate) {
            ...
            foreach ($languageArray as $lang) {
                ...
                if($lang->code2 == Yii::app()->language) {
                    ...
                    $items .= '<li><a class="btn btn-primary btn-xs" data-language="'.$lang->code2.'" href="#" class="ts-current-lang-link" onclick="javascript:return false;">'.$item.'</a></li>';
                } else {
                    $params['_lang'] = $lang->code2;
                    $items .= '<li><a class="btn btn-primary btn-xs" data-language="'.$lang->code2.'" href="'.Yii::app()->createUrl(Yii::app()->controller->route, $params).'">'.$item.'</a></li>';
                }
            }
            if($this->type === 'inline') {
                $currentLang = '';
