...
$url_site_vac = "http://101.com.ua/cgi-bin/srch.cgi?vac=0&razd=";   // резюме первая часть URL
//-----------------------------------------------------------
...
echo "</br></br>Выбор всех ссылок закончен, проверка массива : </br></br>";
    ...
    foreach($url_all as $ist2){
        ...
        $name = $htmlmore->find("td:contains('Соискатель:') + td"); // вытаскиваем phone
        $name = strip_tags($name); // удаляем html - теги
        ...
        $city = $htmlmore->find("td:contains('Регион соискателя:') + td"); // вытаскиваем название города
        ...
        $zp = $htmlmore->find("td:contains('Зарплата:') + td"); // вытаскиваем phone
        $zp = strip_tags($zp); // удаляем html - теги
        $zp = iconv('windows-1251', 'utf-8',$zp);
        echo "Зарплата----|".$zp. "|---</br>";

        // вытаскиваем то, что находится в "Резюме"
        $text = $htmlmore->find("td:contains('Резюме')")->find("td:contains('Резюме') + td"); // вытаскиваем текст
        //$text_2part = explode('Размещено/Обновлено:',$text); // разбиваем на две части
        //$text = $text_2part[0];
        ...
        echo "SELECT id FROM jb_city WHERE city_name = '".$city."' LIMIT 1";
